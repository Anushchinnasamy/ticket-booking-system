    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, JwtAuthenticationFilter jwtAuthenticationFilter) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/actuator/**", "/checkout-test.html").permitAll()
                        // Called by Razorpay Checkout's own JS callback in the browser,
                        // not by a logged-in API client — there's no bearer token to
                        // check here. /verify's real security is the Razorpay
                        // HMAC-SHA256 signature check inside PaymentService, which an
